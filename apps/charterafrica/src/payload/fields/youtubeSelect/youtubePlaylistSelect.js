...
import { getEmbeddedPlaylist, mapPlaylistLinkToId } from "../../utils/embeddedPlaylist";

import YouTubeSelect from "./youtubeSelect";

async function validateYouTubeSelect(value, { data: document, hasMany, required, t }, blockSlug) {
    let options = [];
            ...
            },
            {
                ...
                fields: [
                    ...
                    },
                    {
                        ...
                        options: ["-"],
                        required: true,
                        validate: (value, props) => validateYouTubeSelect(value, props, slug),
                        admin: {
                            description: () => "Enter playlist URL above to select an audio/video",
                            components: {
                                Field: (props) => createElement(YouTubeSelect, { slug, ...props })
                            },
                            condition: (_, siblingData) => siblingData?.featuredType === "custom"
                        }
                    }
                ],
                admin: {
