...
const validateSteps: Validate = (value = [], args: any) => {
  ...
  const missingSteps = requiredSteps.filter(
    (slug) => !value?.find(({ blockType }: { blockType: string }) => blockType === slug),
  )
  if (missingSteps.length) {
    return `The following steps are missing: ${missingSteps.join(', ')}`
  }
  const robotsTxtBlockIndex = value.findIndex(
    ({ blockType }: { blockType: string }) => blockType === 'existing-robots-txt',
  )
...
const ExistingRobots: Block = {
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'text',
      required: true,
      defaultValue: 'Please enter a valid URL. A valid URL should start with http:// or \nhttps://',
    },
    {
      name: 'fetch',
      type: 'text',
...
const Paths: Block = {
  ...
  imageAltText: 'Set disallowed and allowed paths for the robots you want to generate.',
  fields: [
    {
      name: 'title',
      type: 'text',
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: 'text',
          required: true,
          defaultValue: 'The allowed paths directive specifies the paths that a bot should visit.',
        },
      ],
    },
  ],
...
const SiteMaps: Block = {
  ...
  fields: [
    ...
    richText({
      ...
      defaultValue: [
        {
          children: [{ text: 'You can add sitemap URLs to your robots.txt file.' }],
        },
      ],
    }) as unknown as Field,
    {
